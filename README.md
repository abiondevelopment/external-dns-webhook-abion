...
| DOMAIN_FILTER        | Comma-separated list of zones to manage (e.g. `example.com,other.com`). When set, only these zones are fetched from the Abion API, significantly reducing startup time. If unset, all accessible zones are fetched. | Default: (empty)     |
